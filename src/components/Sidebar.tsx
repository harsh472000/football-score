...
export default function Sidebar() {
  ...
  return (
    <aside className="h-[95vh] sm:h-full sm:w-full md:hidden w-[278px] bg-gray-100 dark:bg-backgroundDark text-backgroundDark dark:text-white transition-all rounded-[10px] flex flex-col sticky top-4 sm:hidden">
      {/* Sticky Logo */}
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-primary scrollbar-track-gray-300 dark:scrollbar-track-gray-700">
        <nav className="mt-4 space-y-1 px-3">
          ...
          />
          <SidebarLink
            href="/followed-ground"
            icon={<MapPin size={16} />}
            label="Followed Ground"
            smallText={true}
          />
      ...
      <div className="px-3 py-4 border-t border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-backgroundDark flex flex-col space-y-3 rounded-b-[10px]">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <Link
            href="/logout"
            className="px-2 py-3 bg-gray-200 dark:bg-mainDark rounded-[10px]"
          >
...
const SidebarLink = ({
  ...
  <Link href={href} className="block">
    ...
    >
      {/* Left Accent Bar */}
      {active && (
        <div className="absolute left-0 top-1/2 -translate-y-1/2 h-[80%] w-[7px] bg-primary rounded-br-full rounded-tr-full"></div>
      )}
      <div>{icon}</div>
      <div className="flex items-center justify-between w-full">
        <span className={`font-bold ${smallText && "text-[14px]"}`}>
          {label}
        </span>
